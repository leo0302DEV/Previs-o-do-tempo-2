...
async function searchGeolocateInfo(search) {
  try {
    const response = await fetch(
      `https://dataservice.accuweather.com/locations/v1/cities/search?apikey=${api_key}&q=${search}&offset=1`
    );
...
async function searchWeatherData(response, lang, metric) {
  try {
    ...
    const WeatherInfo = await fetch(
      `https://dataservice.accuweather.com/forecasts/v1/daily/5day/${cityCode}?apikey=${api_key}&language=${lang}&details=true&metric=${metric}`
    );
